...
export default function EditUserPage() {
  ...
  const onFinish = async (values: UserRequest) => {
    ...
    try {
      ...
      if (values.username !== originalValues.username) changes.username = values.username;
      if (values.email !== originalValues.email) changes.email = values.email;
      if (values.phoneNumber !== originalValues.phoneNumber) changes.phoneNumber = values.phoneNumber;
      ...
      form.setFieldsValue(updatedData);
       ...
       const errorMessage = e.response?.data?.message || "Ошибка при обновлении данных";
  ...
  return (
    <Card title="Редактирование профиля пользователя" style={{ maxWidth: 600, margin: "0 auto" }}>
      <Form
        form={form}
        layout="vertical"
        onFinish={onFinish}
